...
use storage::{
    AdminRepo, CommentRepo, Db, DuckRepo, FlockRepo, FollowRepo, NotificationRepo, OidcFlowRepo,
    PhotoStore, SightingRepo, UserRepo, VesselRepo,
};
...
impl AppState {
    ...
    pub fn admin(&self) -> AdminRepo {
        AdminRepo(self.inner.db.pool().clone())
    }
}
